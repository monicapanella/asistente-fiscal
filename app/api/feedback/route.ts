  ...
  try {
    ...
    if (feedback === 'verified') {
      ...
      const { error } = await supabase.from('pending_citations').insert({
        ...
        source: card.source,
        resolution_date: card.date || null,
        criterion: card.criterion,
        keywords: [], // Se completarán en la revisión
        suggested_doctrinal_block: null,
        suggested_block_section: null,
        raw_investigation_data: JSON.stringify({
          original_card: card,
          query_text,
          assistant_type,
      ...
      if (error) {
        ...
        return NextResponse.json(
          { error: 'Error guardando la ficha para revisión' },
          { status: 500 }
        )
      }
      ...
      // En el futuro se puede usar para análisis de calidad
