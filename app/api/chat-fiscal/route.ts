  ...
  while ((match = cardPattern.exec(text)) !== null) {
    try {
      ...
      cards.push({
        ...
        verified: false,     // Se actualiza en el post-processing
        source_level: 2,     // Por defecto nivel 2 (investigación), se actualiza si es verificada
      })
    } catch (parseError) {
      console.warn('⚠️ Error parseando ficha de investigación:', parseError)
      // Fallback: si el JSON no parsea, ignorar esta ficha
      // El texto original ya fue enviado al frontend durante el streaming
    }
  }
...
- \`relevance\`: por qué esta resolución es relevante para el caso concreto
- \`applicability\`: "Vinculante en todo el territorio" | "Vinculante en [comunidad autónoma]" | "Precedente orientativo de otra jurisdicción"
      ...
      async start(controller) {
        ...
        try {
          ...
          if (investigationCards.length > 0) {
            console.log(`📋 Fichas de investigación extraídas: ${investigationCards.length}`)
          ...
          // Enviar como evento structured en vez de texto markdown
          ...
          if (citationNumbers.length > 0) {
            ...
            if (verificationResults.length > 0) {
              // Enviar resultados de verificación como evento estructurado
